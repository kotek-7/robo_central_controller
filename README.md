...
オススメはエントリポイントの [`main.cpp`](https://github.com/kotek-7/robo_central_controller/blob/main/src/main.cpp) から読み始めることです。コードは抽象化されているので、例えば `cat.meow()` や `enemy.kill()` のように、コードの中身は分からなくてもどんな動作をするのかは想像がつく記述がここには多いです。  
