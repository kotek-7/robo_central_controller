...
namespace m3508_controller {
    ...
    public:
        M3508Controller(std::function<void(String)> remote_print,
                        std::function<void(float angle, int16_t rpm, int16_t amp, uint8_t temp)> send_feedback);
        void setup();
        void loop();
    ...
    private:
        ...
        void derive_feedback_fields(const uint8_t rx_buf[8], float *out_angle, int16_t *out_rpm, int16_t *out_amp,
                                    uint8_t *out_temp);
    };
} // namespace m3508_controller